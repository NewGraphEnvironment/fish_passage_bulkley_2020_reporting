...
There is `r my_priority_info() %>% pull(upstream_habitat_length_km)`km of habitat upstream of crossing `r as.character(my_site)` with habitat rated as `r my_priority_info(loc = 'us') %>% pull(hab_value)` value for fry and juvenile salmonid rearing and another `r my_priority_info(sit = my_site2) %>% pull(upstream_habitat_length_km)`km upstream of `r as.character(my_site2)` rated as `r my_priority_info(sit = my_site, loc = 'us') %>% pull(hab_value)`. Although unconfirmed, Grave Creek FSR is likely a tenure road under the responsibility of the Ministry of Forests, Lands, Natural Resource Operations and Rural Development. The crossing was ranked as `r my_priority_info() %>% pull(priority)` priority for proceeding to design for replacement.
