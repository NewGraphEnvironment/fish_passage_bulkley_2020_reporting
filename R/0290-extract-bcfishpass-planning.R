...
##here is the elk
wshd_study_areas_albers <- sf::read_sf(dsn = 'data/fishpass_mapping/fishpass_mapping.gpkg',
...
query <- "SELECT p.*, a.watershed_group_code
  FROM working.misc a
INNER JOIN whse_fish.pscis_assessment_svw p ON (ST_Within(p.geom, a.geom));"

dat_pscis <- st_read(conn, query = query)

query2 <- "SELECT p.*, a.watershed_group_code
  FROM working.misc a
INNER JOIN whse_fish.pscis_habitat_confirmation_svw p ON (ST_Within(p.geom, a.geom));"

dat_pscis2 <- st_read(conn, query = query2)

query3 <- "SELECT p.*, a.watershed_group_code
  FROM working.misc a
INNER JOIN whse_fish.pscis_remediation_svw p ON (ST_Within(p.geom, a.geom));"

dat_pscis3 <- st_read(conn, query = query3)

dbDisconnect(conn = conn)
##burn the pscis info to the project db so that we can do a summary in the report

conn <- rws_connect("data/bcfishpass.sqlite")
rws_list_tables(conn)
rws_write(dat_pscis, exists = F, delete = TRUE,
          conn = conn, x_name = "pscis_historic_phase1")
rws_write(dat_pscis2, exists = F, delete = TRUE,
          conn = conn, x_name = "pscis_historic_phase2")
rws_write(dat_pscis3, exists = F, delete = TRUE,
          conn = conn, x_name = "pscis_historic_phase4")
rws_list_tables(conn)
rws_disconnect(conn)
...
##############haven't actually gotten here yet#########################################################
